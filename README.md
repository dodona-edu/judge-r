...
- `verify_anwser = FALSE` A logical value indicating whether the given anwser should be tested. When set to `TRUE` the judge will tell the student if his anwser is wrong or correct. When set to `FALSE` the judge will only test if the given anwser is valid (contained within `posible_anwsers`).
